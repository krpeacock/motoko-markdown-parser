...
module {
  public func textIndexOf(text: Text, pattern: Text.Pattern, start: Nat) : Int {
    var patternText = "";
    switch (pattern){
      case (#text t) {
        patternText := t;
    ...
    for (i in Iter.range(start, text.size() - patternText.size())) {
  ...
  public func textSubstring(text: Text, start: Int, end: Int) : Text {
    var output = "";
    var count = 0;
